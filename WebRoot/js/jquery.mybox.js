...
#
#	$Id: jquery.mybox.js 9 2012-07-02 15:00:11Z along $
#
			...
			if (_this.EXT.boxType && $.inArray(_this.EXT.boxType, ['iframe', 'ajax','img'])!=-1) {
				...
				if (_this.EXT.boxType === "img") {
					...
					if (_this.EXT.boxType === "ajax") {
						...
						ifr = $("<iframe name='myBoxIframe' style='width:" + _this.EXT.iframe.width + "px;height:" + _this.EXT.iframe.height + "px;' scrolling='no' frameborder='0' src='" + _this.EXT.target + '#t='+ Math.random() + "'></iframe>");
						ifr.appendTo(con.empty());
						ifr.load(function(){
							try {
								it = $(this).contents();
								it.find('.myBox_close').click(_this.close);
								fH = it.height()-60;//iframe height
								fW = it.width();
								w = win;
